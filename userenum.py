...
# 2nd command: who   - Shows who is currenlty logged in
# 3rd command: whoami - Shows who you are logged in as
# 4th command: groups - Shows all groups for the current logged in user
# 5th command: echo $PATH - Shows which directory the current user has access to
# 6th command: grep -e /bin/bash /etc/passwd - Shows any users assigned to the Bash shell
# 7th command: lastlogin Shows all user’s login name, port, and last login time
